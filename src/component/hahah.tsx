...
export default function OutputPanel({ resumeText, jobDesc }: OutputPanelProps) {
  ...
  async function toggleEdit() {
    if (isEditing) {
      ...
      if (aiOutput) {
        ...
        // Save as PDF when saving changes
        await saveAsPDF();
      }
      setIsEditing(false);
    } else {
  ...
  // Save as PDF using html2canvas and pdf-lib
  async function saveAsPDF() {
    if (!outputRef.current) return;
    const canvas = await html2canvas(outputRef.current, { backgroundColor: "#000" });
    const imgData = canvas.toDataURL("image/png");

    const pdf = await PDFDocument.create();
    const page = pdf.addPage();

    // Calculate image size to fit PDF page
    ...
    link.href = url;
    link.download = "resume_output.pdf";
      ...
      {!loading && aiOutput && (
        <div className="space-y-10 text-base leading-relaxed">
          ...
          <button
            ...
            className="mt-4 ml-4 px-6 py-2 bg-white text-black rounded-md font-semibold hover:bg-gray-300"
            disabled={!aiOutput}
          >
            Download as DOCX
          </button>
